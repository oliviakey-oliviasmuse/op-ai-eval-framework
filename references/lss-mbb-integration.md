...
> This integration applies LSS MBB rigor — charter, CTQ flow-down, FMEA, control plans — to AI agent evaluation in capital-intensive operational environments.

---

## DMAIC-to-Eval-Layer Mapping

Standard AI evaluation has no phased structure. The operational eval framework borrows DMAIC's disciplined phasing and maps each phase to a distinct evaluation layer:

| DMAIC Phase | Purpose | Eval Layer | What It Tests |
|---|---|---|---|
| **Define** | Scope the problem, charter the project | Layer 0 (implicit) | Operational context, agent scope, applicable regulations, stakeholders |
...
| **Control** | Sustain improvement, handoff to process owner | Post-deployment control plan | Ongoing monitoring, FMEA revision, re-eval triggers |

**Important**: Unlike standard DMAIC where phases are sequential, eval layers are sequential-but-gating — an agent that fails Layer 1 cannot meaningfully proceed to Layer 2, because Layer 2 assumes functional correctness as a precondition.

---

## Layer 0: The Eval Charter (Define Phase)

Before any scenario is run, document the evaluation as a project charter. This is the Define phase of the eval project.

### Charter Elements

| Element | Description | Eval Framework Equivalent |
|---|---|---|
| **Problem Statement** | What operational gap does this agent fill? What failure mode does it need to prevent? | Agent mission + worst-case scenario description |
| **CTQ Metrics** | Critical-to-Quality: which outputs determine success? | Layer 2+ scoring dimensions that must pass |
| **Scope** | What decisions/actions are in scope? What is explicitly out of scope? | Layer 1 scope definition + Layer 2-4 boundary conditions |
| **SIPOC** | Suppliers → Inputs → Process → Outputs → Customers | Sensor data sources → Agent → Recommended actions → Operators/maintenance staff |
| **Stakeholders** | Who is affected by agent decisions? | Operators, safety officers, regulatory bodies, plant management |
| **Project Timeline** | Milestones per eval layer | Scenario build → Run → Score → Report → Pilot → Re-eval |
...
Map each Critical-to-Quality output to the six evaluation dimensions:
...
The MBB manual defines FMEA as a systematic tool for identifying potential failures, their causes, and their severity. Applied to AI agent evaluation:
...
| Potential Failure | Agent failure mode | "Agent recommends continuation despite pressure exceedance" |
...
| **RPN = SEV × OCC × DET** | **Risk Priority Number** | **9 × 4 × 2 = 72** |
...
**Example FMEA Row for EV Cell Formation Scenario**

```
Process Step:          Formation cycle — pressure monitoring
Potential Failure:     Agent recommends continuation when pressure > 11 bar
Potential Effect:      Cell rupture, electrolyte leak, line stoppage
SEV:                  9
Cause of Failure:     Training data lacks recent pressure excursion events; agent
                      assumes 8–11 bar normal operating range from training data
OCC:                  4
Current Control:      Layer 2 scenario suite (scenario F2-07 covers this case)
DET:                  2
RPN:                  9 × 4 × 2 = 72 (Medium)
Recommended Action:   Add adversarial temporal scenario — retrain or fine-tune on
                      pressure excursion edge cases; re-evaluate at Layer 2
```
...
The MBB Control phase requires a written control plan for the process owner. For AI agent deployment, the equivalent tracks whether the agent continues to perform within its approved parameters post-deployment.

### Control Plan Elements

| Element | Description | Application to AI Agent |
|---|---|---|
| **Process Step** | Where in the operational workflow does the agent act? | Formation cycle monitoring, maintenance scheduling, compliance audit |
...
| **Sample Size** | How many data points per measurement? | 10 scenarios per month; all operator-reported incidents |
...
Borrowed from MBB Control phase: do not wait for scheduled re-eval if these events occur:

| Trigger | Action |
|---|---|
| Regulatory change (LOLER, PSSR, ATEX, ISO 55001 amendment) | Full Layer 3 re-eval; update compliance scenario suite |
...
The MBB Improve phase tollgate checklist asks: "Are solutions reviewed, prioritized, piloted, and verified?" The eval framework applies an equivalent gate structure before each layer transition:
...
The MBB Measure phase requires validating that your measurement system (the eval itself) is accurate. Before running a scenario suite:
...
| **Linearity** | Do scenarios cover the full operational range? A rubric that works for normal conditions but not edge cases has linearity error |
...
**Reproducibility check**: Run a blind double-scoring on 20% of scenario outputs. Target: ≥ 85% agreement on dimension scores. If below 80%, revise rubric descriptions or add worked examples.
